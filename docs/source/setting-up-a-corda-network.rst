...
to every other node so that they can all see each other.
...
in the other nodes' ``additional-node-infos`` directory. A simple way to do this is to use rsync. If it's known beforehand
the set of nodes that will eventually join they can be pregenerated in the bootstrap and only started when needed.
