...
be moved to another machine open its config file and make the following changes:
...
Once that's done move the node folders to their relevant machines (e.g. using a USB key). It is important that none of the
